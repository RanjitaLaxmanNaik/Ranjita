...
class payment:
    def __init__(self,root):
        ...
        lbl_title=Label(self.root,text="Payment Details",font=("times new roman",18,"bold"),bg="black",fg="gold",bd=4,)
        lbl_title.place(x=0,y=0,width=1295,height=50)

        
        
           #****************************label Frame***************************#
        labelframeleft=LabelFrame(self.root,bd=2,text="Payment details",font=("times new roman",16,"bold"),padx=2)
        labelframeleft.place(x=5,y=50,width=425,height=490)

        #*****************************label entries*******************************#
        lbl_cust_ref=Label(labelframeleft,text="House No",font=("times new roman",12,"bold"),padx=2,pady=6)
        lbl_cust_ref.grid(row=0,column=0,sticky=W)
        enty_ref=ttk.Entry(labelframeleft,textvariable=self.var_houseid,font=("times new roman",13,"bold"),width=29)
        enty_ref.grid(row=0,column=1)

        

        cname=Label(labelframeleft,text="tenantname",font=("times new roman",12,"bold"),padx=2,pady=6)
        ...
        lblmname=Label(labelframeleft,text="Invoice",font=("times new roman",12,"bold"),padx=2,pady=6)
        lblmname.grid(row=2,column=0,sticky=W)
        txtmname=ttk.Entry(labelframeleft,textvariable=self.var_invoice,font=("arial",13,"bold"),width=29)
        txtmname.grid(row=2,column=1)

        lblmname=Label(labelframeleft,text="Date",font=("times new roman",12,"bold"),padx=2,pady=6)
        lblmname.grid(row=3,column=0,sticky=W)
        txtmname=ttk.Entry(labelframeleft,textvariable=self.var_date,font=("arial",13,"bold"),width=29)
        txtmname.grid(row=3,column=1)


        
        
        lblPostCode=Label(labelframeleft,text="Amount",font=("times new roman",12,"bold"),padx=2,pady=6)
        lblPostCode.grid(row=4,column=0,sticky=W)
        txtPostCode=ttk.Entry(labelframeleft,textvariable=self.var_amount,font=("arial",13,"bold"),width=29)
        ...
        combo_Search["value"]=("Fname","Invoice")
        ...
        self.Cust_Details_Table=ttk.Treeview(details_table,column=("houseno","fname","invoice","date","amount"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        scroll_x.pack(side=BOTTOM,fill=X)
        scroll_y.pack(side=RIGHT,fill=Y)
        scroll_x.config(command=self.Cust_Details_Table.xview)
        scroll_y.config(command=self.Cust_Details_Table.yview)

        self.Cust_Details_Table.heading("houseno",text="HouseId")
        
        self.Cust_Details_Table.heading("fname",text="Fname")
        self.Cust_Details_Table.heading("invoice",text="Invoice")
        ...
        self.Cust_Details_Table.column("invoice",width=100)
        self.Cust_Details_Table.column("date",width=100)
    ...
    def add_data(self):
        if self.var_amount.get()=="" or self.var_invoice.get()=="":
    ...
    def Update(self):
        if self.var_house_no.get()=="":
            messagebox.showerror("error","Please enter the name",parent=self.root)
        else:

            conn=mysql.connector.connect(host="localhost",username="root",password="root@1234",database="management")
            my_cursor=conn.cursor()
            my_cursor.execute("update customer set Fname=%s,Invoice=%s,Date=%s,Amount=%s where HouseId=%s",(
    ...
    def Delete(self):
        Delete=messagebox.askyesno("house rental management","do you want to delete the payment",parent=self.root)
        if Delete>0:
            conn=mysql.connector.connect(host="localhost",username="root",password="root@1234",database="management")
            my_cursor=conn.cursor()
            query="delete from payment where HouseId=%s"
            value=(self.var_houseid.get(),)
